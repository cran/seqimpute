...
test_that("MICT works without error", {
  skip_on_cran()
  expect_no_error(seqimpute(gameadd.traj, nfi = 0, m = 2))
})

test_that("MICT-timing works without error", {
  skip_on_cran()
  expect_no_error(seqimpute(gameadd.traj, timing = TRUE, nfi = 0, m = 2))
})

test_that("MICT works without error", {
  skip_on_cran()
  expect_no_error(seqimpute(gameadd.traj, npt = 0, m = 2))
})

test_that("MICT-timing works without error", {
  skip_on_cran()
  expect_no_error(seqimpute(gameadd.traj, timing = TRUE, npt = 0, m = 2))
})
...
test_that("MICT-timing", {
  skip_on_cran()
  expect_message(
    seqimpute(imp$imp[[1]], timing = TRUE, m = 2),
    "This dataset has no missing values!"
  )
})
...
test_that("factor returns factor", {
  skip_on_cran()
  expect_s3_class(seqimpute(gameadd.traj,
    m = 2,
    timing = TRUE
  )$imp[[1]][, 1], "factor")
})
...
test_that("non missing values from the dataset not changed by the imputation", {
  skip_on_cran()
  expect_equal(
    imp.MICT$imp[[1]][!is.na(gameadd.traj)],
...
test_that("non missing values from the dataset not changed by the imputation", {
  skip_on_cran()
  expect_equal(
    imp.MICT$imp[[2]][!is.na(gameadd.traj)],
    gameadd.traj[!is.na(gameadd.traj)]
  )
})
...
test_that("non missing values from the dataset not changed by the imputation", {
  skip_on_cran()
  expect_equal(
    imp.MICTt$imp[[1]][!is.na(gameadd.traj)],
    gameadd.traj[!is.na(gameadd.traj)]
  )
})
...
test_that("non missing values from the dataset not changed by the imputation", {
  skip_on_cran()
  expect_equal(
    imp.MICTt$imp[[2]][!is.na(gameadd.traj)],
    gameadd.traj[!is.na(gameadd.traj)]
  )
})
...
test_that("MICT with covariates and time-varying covariates", {
  skip_on_cran()
  expect_no_error(seqimpute(gameadd,
    var = 1:4, m = 2, covariates = c("Gender", "Age"),
...
test_that("MICT with covariates and time-varying covariates", {
  skip_on_cran()
  expect_no_error(seqimpute(gameadd.cov,
    var = 1:4, m = 2, covariates = c("Gender", "Age"),
...
mvad.miss[7, 1:5] <- NA
mvad.miss[7, 70] <- NA
test_that("no internal", {
  expect_no_error(seqimpute(mvad.miss, regr = "multinom", m = 1, timing = T, nf = 1, np = 1))
})


mvad.miss <- mvad[, 17:86]
mvad.miss[c(3, 4), 15:17] <- NA
test_that("one column per order", {
  expect_no_error(seqimpute(mvad.miss, regr = "multinom", m = 1, timing = T, nf = 1, np = 1))
})

mvad.miss[c(3), 15:17] <- NA
test_that("one column per order", {
  expect_no_error(seqimpute(mvad.miss, regr = "multinom", m = 1, timing = T, nf = 1, np = 1))
})
